...
- Current version: `0.1.4.0` (CMake is the source of truth; the executable embeds matching VERSIONINFO metadata).
- Branch: `master`.
...
- Watermark: the fixed logo is masked by a shared viewer-background backing rectangle beneath every image, so transparent pixels reveal canvas rather than the watermark.
