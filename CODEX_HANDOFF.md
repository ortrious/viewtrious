...
- Current version: `0.2.3.1` (CMake is the source of truth; the executable embeds matching VERSIONINFO metadata).
- Branch: `master`.
...
- Image actions: right-clicking a loaded image opens a FeatherView-rendered context menu with content-driven height. Open With (`SHOpenWithDialog`), Copy (alpha-capable top-down `CF_DIBV5` pixels), and Print (Shell `print` verb) are enabled; rotate, personalization, and Delete remain visibly disabled. Ctrl+C and Ctrl+P use the same action paths. Successful copy displays a centered 104-DIP FeatherView-blue, rounded overlapping-squares indicator and `Copied to Clipboard`, fading together for about one second using a timer that exists only during feedback. Open With pull-out submenu behavior remains deferred.
