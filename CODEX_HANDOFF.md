...
- Top bar: a 40-DIP custom title bar contains a visual-only hamburger, decoded resolution, cheap `GetFileAttributesExW` file-size metadata, and an ellipsized filename, followed by custom caption controls. Resolution and file size use stable centered slots; the filename takes remaining width and is trimmed first. The hamburger popup menu remains deferred.
