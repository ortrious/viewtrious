...
constexpr UINT kVideoStepHoldIntervalMs = 16; // Unsupported reverse-hold seek fallback only.
...
public:
    ...
    HRESULT Initialize(const std::wstring& path) {
        ...
        DWORD visualStyle = static_cast<DWORD>(ModelVisualStyle::ShadedWithVisibleEdges);
        modelVisualStyle_ = ReadSetting(L"ModelVisualStyle", visualStyle) && visualStyle <= static_cast<DWORD>(ModelVisualStyle::Wireframe) ? static_cast<ModelVisualStyle>(visualStyle) : ModelVisualStyle::ShadedWithVisibleEdges;
    ...
    bool VideoUserPlaybackActive() const { return videoPlayer_.Playing() && !videoStepHoldTransportActive_; }
    ...
    void StopVideoStepHold() {
        ...
        if (videoStepHoldAudioSuppressed_ && videoPlayer_.Active()) videoPlayer_.SetMuted(videoMuted_);
        videoStepHoldAudioSuppressed_ = false;
    ...
    bool BeginVideoStepHoldTransport() {
        if (!videoStepHoldDirection_ || !VideoActive() || videoPlayer_.Playing()) return false;
        const double rate = videoStepHoldDirection_ < 0 ? -1.0 : 1.0;
        if (rate < 0.0 && !videoPlayer_.SupportsNegativePlaybackRate()) return false;
        if (!videoPlayer_.SetMuted(true)) return false;
        videoStepHoldAudioSuppressed_ = true;
        if (!videoPlayer_.BeginTemporaryPlayback(rate)) {
    ...
    void UpdateVideoStepHold() {
        KillTimer(window_, kVideoStepHoldTimer);
        if (!videoStepHoldDirection_ || !VideoActive()) { StopVideoStepHold(); return; }
        if (!videoStepHoldActive_) {
            ...
            BeginReverseVideoStepHoldSeekFallback();
            return;
        }
        if (videoStepHoldTransportActive_) return;
        if (videoPlayer_.Playing()) { StopVideoStepHold(); return; }
        LARGE_INTEGER now{};
        if (!QueryPerformanceCounter(&now) || videoStepHoldQpcFrequency_ <= 0) { StopVideoStepHold(); return; }
    ...
    VideoControlsLayout GetVideoControlsLayout(bool) const {
        ...
        const int buttonWidth = std::min({ MulDiv(32, dpi, 96), std::max(MulDiv(24, dpi, 96), height - padding * 2 - timelineHeight - rowGap), std::max(MulDiv(24, dpi, 96), width / 10) });
    ...
    GifControlsLayout GetGifControlsLayout() const {
        ...
        // independent GIF island never shifts when the strip's visibility animation changes.
        const LONG bottom = canvas.bottom - (FilmstripEligible() ? FilmstripHeight() + canvasBottomMargin + filmstripClearance : canvasBottomMargin);
    ...
    void UpdateVideoAdjustmentSlider(int index, POINT point) {
        ...
        const float continuous = minimum + normalized * static_cast<float>(maximum - minimum);
        int value = std::clamp(static_cast<int>(std::lround(continuous)), minimum, maximum);
        const int nearestDetent = std::clamp(static_cast<int>(std::lround(continuous / 5.0f)) * 5, minimum, maximum);
        const bool zeroDetent = nearestDetent == 0;
        const float attraction = zeroDetent ? 1.75f : 0.30f;
        const float hold = zeroDetent ? 3.25f : 1.10f;
        if (videoAdjustmentDetentIndex_ == index && std::abs(continuous - videoAdjustmentDetentValue_) <= hold) {
            value = videoAdjustmentDetentValue_;
        } else if (std::abs(continuous - nearestDetent) <= attraction) {
            value = nearestDetent;
            videoAdjustmentDetentIndex_ = index;
            videoAdjustmentDetentValue_ = nearestDetent;
        } else {
    ...
    void UpdateImageAdjustmentSlider(int index, POINT point) {
        ...
        const int minimum = index == 6 ? 0 : -100;
        const float continuous = minimum + position * static_cast<float>(100 - minimum);
        int integer = std::clamp(static_cast<int>(std::lround(continuous)), minimum, 100);
        const int detent = std::clamp(static_cast<int>(std::lround(continuous / 5.0f)) * 5, minimum, 100);
        const bool zeroDetent = detent == 0;
    ...
    void UpdateVideoControlsMouse(POINT point) {
        ...
        if (!VideoUserPlaybackActive() || revealZone || activeInteraction) ShowVideoControls(false);
        ...
        else if (wasPointerOver && VideoUserPlaybackActive()) {
            videoControlsFadeActive_ = false;
            videoControlsLastActivity_ = GetTickCount64();
            SetTimer(window_, kVideoControlsTimer, static_cast<UINT>(kVideoControlsIdleDelayMs), nullptr);
        }
    ...
    void VideoControlsMouseLeave() {
        ...
        if (VideoUserPlaybackActive() && !videoScrubbing_ && !videoVolumeDragging_ && !videoAdjustmentsOriginalPreviewActive_ && !videoPlaybackSpeedPanelOpen_) {
            videoControlsFadeActive_ = false;
            videoControlsLastActivity_ = GetTickCount64();
            SetTimer(window_, kVideoControlsTimer, static_cast<UINT>(kVideoControlsIdleDelayMs), nullptr);
        }
    ...
    void UpdateVideoControlsFade() {
        ...
        if (!VideoUserPlaybackActive() || videoControlsPointerOver_ || videoScrubbing_ || videoVolumeDragging_ || videoAdjustmentsOriginalPreviewActive_ || videoPlaybackSpeedPanelOpen_) { KillTimer(window_, kVideoControlsTimer); return; }
    ...
    int ModelViewBarPlateWidth() const { return std::max(MulDiv(160,GetDpiForWindow(window_),96),MeasureSettingsTextWidth(L"5000 \u00D7 5000 mm",14,DWRITE_FONT_WEIGHT_SEMI_BOLD)+MulDiv(24,GetDpiForWindow(window_),96)); }
    ...
    RECT GetBuildPlateSizePopupBounds() const { const RECT bar=GetModelViewBarPlateBounds(); const int dpi=GetDpiForWindow(window_); const int width=MulDiv(230,dpi,96),height=MulDiv(124,dpi,96); return {std::max(ModelCanvasBounds().left,bar.left),bar.bottom+MulDiv(4,dpi,96),std::max(ModelCanvasBounds().left,bar.left)+width,bar.bottom+MulDiv(4,dpi,96)+height}; }
    RECT GetBuildPlateSizeFieldBounds(int edit) const { const RECT popup=GetBuildPlateSizePopupBounds(); const int dpi=GetDpiForWindow(window_),row=MulDiv(30,dpi,96); const int left=popup.left+MulDiv(88,dpi,96),right=popup.right-MulDiv(44,dpi,96); return {left,popup.top+row*edit+MulDiv(3,dpi,96),right,popup.top+row*(edit+1)-MulDiv(3,dpi,96)}; }
    RECT GetBuildPlateSizeLinkBounds() const { const RECT popup=GetBuildPlateSizePopupBounds(); const int dpi=GetDpiForWindow(window_),row=MulDiv(30,dpi,96); return {popup.right-MulDiv(40,dpi,96),popup.top+row,popup.right-MulDiv(4,dpi,96),popup.top+row*3}; }
    bool CommitBuildPlateSize(bool width) { wchar_t* end=nullptr; const float value=std::wcstof(buildPlateSizeText_.c_str(),&end); if(end==buildPlateSizeText_.c_str()||*end||!std::isfinite(value)||value<50||value>5000)return false; if(width)modelBuildPlateWidthMm_=value;else modelBuildPlateDepthMm_=value;if(buildPlateSizeLinked_){modelBuildPlateWidthMm_=modelBuildPlateDepthMm_=value;}WriteSetting(L"ModelBuildPlateWidthMm",DWORD(std::lround(modelBuildPlateWidthMm_)));WriteSetting(L"ModelBuildPlateDepthMm",DWORD(std::lround(modelBuildPlateDepthMm_)));if(ModelActive())modelViewport_.SetBuildPlate(BuildPlateVisible(),ModelUpVector(),modelBuildPlateWidthMm_,modelBuildPlateDepthMm_);InvalidateRect(window_,nullptr,FALSE);return true; }
    void CommitActiveBuildPlateSize() { if(!buildPlateSizeEdit_)return; CommitBuildPlateSize(buildPlateSizeEdit_==1); buildPlateSizeEdit_=0; buildPlateSizeReplace_=false; InvalidateRect(window_,nullptr,FALSE); }
    void FocusBuildPlateSize(int edit) { if(buildPlateSizeEdit_&&buildPlateSizeEdit_!=edit)CommitActiveBuildPlateSize();buildPlateSizeEdit_=edit;buildPlateSizeReplace_=true;buildPlateSizeText_=std::to_wstring(int(std::lround(edit==1?modelBuildPlateWidthMm_:modelBuildPlateDepthMm_)));InvalidateRect(window_,nullptr,FALSE); }
    void CancelBuildPlateSizeEdit() { buildPlateSizeEdit_=0;buildPlateSizeReplace_=false;buildPlateSizeText_.clear(); }
    void NavigateBuildPlateSize(bool reverse) { const int next=buildPlateSizeEdit_==0?(reverse?2:1):buildPlateSizeEdit_==1?2:1;FocusBuildPlateSize(next); }
    void BuildPlateSizeChar(wchar_t character) { if(!buildPlateSizeEdit_)return; if(character==L'\b'){buildPlateSizeReplace_=false;if(!buildPlateSizeText_.empty())buildPlateSizeText_.pop_back();}else if(character>=L'0'&&character<=L'9'){if(buildPlateSizeReplace_){buildPlateSizeText_.clear();buildPlateSizeReplace_=false;}if(buildPlateSizeText_.size()<4)buildPlateSizeText_+=character;}else if(character==L'\r'){CommitActiveBuildPlateSize();}InvalidateRect(window_,nullptr,FALSE); }
    ...
    void SetModelBuildPlate(ModelBuildPlate mode) { if (modelBuildPlate_ == mode) return; const bool wasVisible=BuildPlateVisible(); modelBuildPlate_ = mode; WriteSetting(L"ModelBuildPlate", static_cast<DWORD>(mode)); const bool visible=BuildPlateVisible(); if (!visible) DismissBuildPlateSizePopup(); if (visible&&!wasVisible&&ModelActive()) { std::wstring error; if (!modelViewport_.CenterModelOnBuildPlate(ModelUpVector(),error)) error_=error; } if (ModelActive()) modelViewport_.SetBuildPlate(visible, ModelUpVector(), modelBuildPlateWidthMm_, modelBuildPlateDepthMm_); InvalidateRect(window_, nullptr, FALSE); }
    ...
    ButtonKind ButtonAt(POINT point) const {
        ...
        if (ModelActive()) {
            ...
            if(BuildPlateVisible()&&buildPlateSizePopupOpen_){const RECT popup=GetBuildPlateSizePopupBounds(),width=GetBuildPlateSizeFieldBounds(1),depth=GetBuildPlateSizeFieldBounds(2),link=GetBuildPlateSizeLinkBounds();if(!PtInRect(&popup,point))return ButtonKind::None;if(PtInRect(&width,point))return ButtonKind::ViewBarPlateWidth;if(PtInRect(&depth,point))return ButtonKind::ViewBarPlateDepth;if(PtInRect(&link,point))return ButtonKind::ViewBarPlateLink;const int row=MulDiv(30,GetDpiForWindow(window_),96);return point.y>=popup.top+row*3?ButtonKind::ViewBarPlateReset:ButtonKind::None;}
    ...
    void InvokeButton(ButtonKind button) {
        ...
        else if (button == ButtonKind::ViewBarBuildPlateSize) { if(buildPlateSizePopupOpen_){CommitActiveBuildPlateSize();DismissBuildPlateSizePopup();}else{buildPlateSizePopupOpen_=true;buildPlateSizeEdit_=0;buildPlateSizeReplace_=false;InvalidateRect(window_,nullptr,FALSE);} }
        else if (button == ButtonKind::ViewBarPlateWidth) FocusBuildPlateSize(1);
        else if (button == ButtonKind::ViewBarPlateDepth) FocusBuildPlateSize(2);
        else if (button == ButtonKind::ViewBarPlateLink) { CommitActiveBuildPlateSize(); buildPlateSizeLinked_=!buildPlateSizeLinked_; WriteSetting(L"ModelBuildPlateSizeLinked",buildPlateSizeLinked_?1:0); InvalidateRect(window_,nullptr,FALSE); }
        else if (button == ButtonKind::ViewBarPlateReset) { CommitActiveBuildPlateSize(); modelBuildPlateWidthMm_=modelBuildPlateDepthMm_=256; WriteSetting(L"ModelBuildPlateWidthMm",256);WriteSetting(L"ModelBuildPlateDepthMm",256);if(ModelActive())modelViewport_.SetBuildPlate(BuildPlateVisible(),ModelUpVector(),256,256);InvalidateRect(window_,nullptr,FALSE); }
    ...
    void ClampFilmstripScrollToViewport() {
    ...
    void RebuildFilmstripLayout(bool clampScroll = true, bool queueThumbnails = true) {
        ...
        if (clampScroll) ClampFilmstripScrollToViewport();
    ...
    double FilmstripQpcElapsedMs(LONGLONG started, LONGLONG frequency) const {
        if (!started || frequency <= 0) return 0.0;
        LARGE_INTEGER now{};
        QueryPerformanceCounter(&now);
        return static_cast<double>(now.QuadPart - started) * 1000.0 / static_cast<double>(frequency);
    }
    void SampleFilmstripHoverVisual() {
        if (!filmstripHoverVisualAnimating_) return;
        const float progress = static_cast<float>(std::clamp(FilmstripQpcElapsedMs(filmstripHoverVisualStartedQpc_, filmstripHoverVisualQpcFrequency_) /
            kFilmstripHoverVisualDurationMs, 0.0, 1.0));
        filmstripHoverVisualProgress_ = filmstripHoverVisualStartProgress_ +
            (filmstripHoverVisualTargetProgress_ - filmstripHoverVisualStartProgress_) * SmoothTransitionProgress(progress);
        if (progress >= 1.0f) {
            filmstripHoverVisualAnimating_ = false;
            KillTimer(window_, kFilmstripHoverVisualTimer);
            if (filmstripHoverVisualProgress_ <= 0.001f) filmstripHoverVisualIndex_ = -1;
        }
    }
    void SetFilmstripHoverVisual(int index) {
        SampleFilmstripHoverVisual();
        if (index != filmstripHoverVisualIndex_) {
            if (index < 0) {
                filmstripHoverVisualStartProgress_ = filmstripHoverVisualProgress_;
            } else {
                filmstripHoverVisualIndex_ = index;
        ...
        filmstripHoverVisualTargetProgress_ = index >= 0 ? 1.0f : 0.0f;
        if (filmstripHoverVisualStartProgress_ == filmstripHoverVisualTargetProgress_) return;
        LARGE_INTEGER frequency{}, now{};
        if (!QueryPerformanceFrequency(&frequency) || !QueryPerformanceCounter(&now)) {
            filmstripHoverVisualProgress_ = filmstripHoverVisualTargetProgress_;
            if (filmstripHoverVisualProgress_ <= 0.001f) filmstripHoverVisualIndex_ = -1;
            return;
        }
        filmstripHoverVisualStartedQpc_ = now.QuadPart;
        filmstripHoverVisualQpcFrequency_ = frequency.QuadPart;
        filmstripHoverVisualAnimating_ = true;
        SetTimer(window_, kFilmstripHoverVisualTimer, 15, nullptr);
    }
    void SetFilmstripHover(POINT point) {
        if (filmstripWrapFade_.active) return;
        if (FilmstripHoverSuppressedForImagePan()) {
            SuppressFilmstripHoverForImagePan();
            return;
        }
    ...
    bool FilmstripHoverPreviewDrawable() const {
        return filmstripPreviewIndex_ >= 0 && !filmstripDragging_ && !filmstripScrollAnimating_ &&
            (FilmstripHoverPreviewEligible(filmstripPreviewIndex_) || (filmstripHoverPreviewFadeActive_ && filmstripHoverPreviewFadeOut_));
    }
    void SynchronizeFilmstripHoverPreviewAvailability() {
        if (FilmstripHoverPreviewsFitWindow()) return;
        ++filmstripHoverPreviewGeneration_;
        videoHoverPreviewGeneration_.store(filmstripHoverPreviewGeneration_, std::memory_order_release);
        CancelFilmstripVideoHoverFade();
        KillTimer(window_, kFilmstripHoverPreviewTimer);
        KillTimer(window_, kFilmstripHoverPreviewDwellTimer);
        CancelQueuedFilmstripHoverPreviews();
        if (filmstripPreviewIndex_ >= 0 && (!filmstripHoverPreviewFadeActive_ || !filmstripHoverPreviewFadeOut_))
    ...
    void DrawFilmstrip() {
        ...
        const float contentOpacity = opacity * (drawingLowerUiContents_ ? 1.0f : FilmstripContentOpacity());
        SampleFilmstripHoverVisual();
        const bool dark = UseDarkAppMode();
        const bool drawPreviewShell = !drawingLowerUiContents_ && !filmstripWrapFade_.active && FilmstripHoverPreviewDrawable() && filmstripHoverPreviewOpacity_ > 0.001f;
        ...
        if (FAILED(renderTarget_->CreateSolidColorBrush(AdjustmentSurfaceFill(dark, opacity), &surface)) ||
            ...
            FAILED(renderTarget_->CreateSolidColorBrush(D2D1::ColorF(0.f / 255, 90.f / 255, 160.f / 255, 0.22f * contentOpacity), &selectedBacking)) ||
            FAILED(renderTarget_->CreateSolidColorBrush(D2D1::ColorF(0.f / 255, 120.f / 255, 212.f / 255, 0.25f * contentOpacity), &selectedGlow)) ||
            FAILED(renderTarget_->CreateSolidColorBrush(D2D1::ColorF(0.f / 255, 150.f / 255, 255.f / 255, contentOpacity), &selectedOutline)) ||
            FAILED(renderTarget_->CreateSolidColorBrush(D2D1::ColorF(1.f, 1.f, 1.f, 0.28f * contentOpacity), &hover)) ||
            FAILED(renderTarget_->CreateSolidColorBrush(D2D1::ColorF(1.f, 1.f, 1.f, 0.045f * contentOpacity), &hoverSurface)) ||
            FAILED(renderTarget_->CreateSolidColorBrush(D2D1::ColorF(24.f / 255, 26.f / 255, 30.f / 255, contentOpacity), &placeholder)) ||
            FAILED(renderTarget_->CreateSolidColorBrush(D2D1::ColorF(1.f, 1.f, 1.f, 0.42f * contentOpacity), &placeholderText))) return;
        ...
        for (size_t index = first; index < last; ++index) {
            ...
            const float hoverProgress = !drawingLowerUiContents_ && index == static_cast<size_t>(filmstripHoverVisualIndex_) ? filmstripHoverVisualProgress_ : 0.0f;
            const float hoverScale = 1.0f + 0.035f * hoverProgress;
            const float hoverLift = 2.0f * scale * hoverProgress;
            const float centerX = (box.left + box.right) * 0.5f;
            const float centerY = (box.top + box.bottom) * 0.5f - hoverLift;
            const D2D1_RECT_F visualBox = D2D1::RectF(centerX - (box.right - box.left) * hoverScale * 0.5f,
                centerY - (box.bottom - box.top) * hoverScale * 0.5f, centerX + (box.right - box.left) * hoverScale * 0.5f,
                centerY + (box.bottom - box.top) * hoverScale * 0.5f);
            const D2D1_RECT_F selection = D2D1::RectF(visualBox.left - 4.0f * scale, visualBox.top - 4.0f * scale, visualBox.right + 4.0f * scale, visualBox.bottom + 4.0f * scale);
            if (index == current) {
                ...
                hover->SetColor(D2D1::ColorF(1.f, 1.f, 1.f, 0.28f * contentOpacity * hoverProgress));
                renderTarget_->DrawRoundedRectangle(D2D1::RoundedRect(visualBox, 6.0f * scale, 6.0f * scale), hover.Get(), (1.0f + hoverProgress) * scale);
            }
            renderTarget_->FillRoundedRectangle(D2D1::RoundedRect(visualBox, 6.0f * scale, 6.0f * scale), placeholder.Get());
            ...
            if (hoverProgress > 0.001f) {
                hoverSurface->SetColor(D2D1::ColorF(1.f, 1.f, 1.f, 0.045f * contentOpacity * hoverProgress));
                renderTarget_->FillRoundedRectangle(D2D1::RoundedRect(visualBox, 6.0f * scale, 6.0f * scale), hoverSurface.Get());
            }
            if (IsVideoPath(navigationFiles_[index].wstring())) {
                ...
                const float left = visualBox.right - diameter - 5.0f * scale;
                const float top = visualBox.bottom - diameter - 5.0f * scale;
                const UINT iconSize = static_cast<UINT>(std::max(1.0f, std::round(diameter)));
                if (EnsureFilmstripVideoIcon(iconSize))
                    renderTarget_->DrawBitmap(filmstripVideoIcon_.Get(), D2D1::RectF(left, top, left + diameter, top + diameter), contentOpacity,
                        D2D1_BITMAP_INTERPOLATION_MODE_LINEAR);
            }
            if (index == current) renderTarget_->DrawRoundedRectangle(D2D1::RoundedRect(visualBox, 6.0f * scale, 6.0f * scale), selectedOutline.Get(), 2.0f * scale);
        }
        renderTarget_->PopAxisAlignedClip();
    ...
    bool BeginFilmstripWrapFade(int direction, bool immediatePaint) {
        ...
        if (filmstripWrapFade_.active) {
            ...
            SampleFilmstripWrapFade();
            filmstripWrapFade_.direction = direction;
            filmstripWrapFade_.immediatePaint = immediatePaint;
            filmstripWrapFade_.startOpacity = filmstripWrapFade_.opacity;
        } else {
            if (!FilmstripNavigationWraps(direction) || !FilmstripVisible()) return false;
            filmstripWrapFade_.active = true;
            filmstripWrapFade_.direction = direction;
            filmstripWrapFade_.immediatePaint = immediatePaint;
            filmstripWrapFade_.startOpacity = 1.0f;
        }
        ++filmstripHoverPreviewGeneration_;
        videoHoverPreviewGeneration_.store(filmstripHoverPreviewGeneration_, std::memory_order_release);
        CancelFilmstripVideoHoverFade();
        ...
        KillTimer(window_, kFilmstripHoverVisualTimer);
        LARGE_INTEGER frequency{}, now{};
        if (!QueryPerformanceFrequency(&frequency) || !QueryPerformanceCounter(&now)) { CancelFilmstripWrapFade(); return false; }
        filmstripWrapFade_.qpcFrequency = frequency.QuadPart;
        filmstripWrapFade_.startedQpc = now.QuadPart;
        filmstripWrapFade_.targetOpacity = 0.0f;
        SetTimer(window_, kFilmstripWrapFadeTimer, 15, nullptr);
        InvalidateRect(window_, nullptr, FALSE);
        return true;
    }
    void UpdateFilmstripWrapFade() {
        ...
        if (!state.dispatched) {
            state.opacity = 0.0f;
            // The only intentional scroll discontinuity: make the wrapped end ready
    ...
    void ZoomVideoToActualPixels(POINT cursor) {
        ...
        if (std::abs(actualScale - VideoFitScale()) <= 0.0001f) {
            ...
            videoPan_ = D2D1::Point2F();
            InvalidateRect(window_, nullptr, FALSE);
        } else SetVideoScaleAt(cursor, actualScale);
        ShowVideoControls();
    }
    void ToggleVideoFitActualPixels(POINT cursor) {
        const float physicalScale = VideoCurrentScale() * RenderTargetDpi() / 96.0f;
        if (videoFitToWindow_ || physicalScale < 1.0f - 0.0001f) ZoomVideoToActualPixels(cursor);
        else FitToWindow();
    }
...
public:
    void VideoMediaEngineEvent(DWORD event, uint64_t callbackOpenAttemptId) {
        ...
        if (!videoStepHoldTransportActive_) videoEffectivePlaybackRate_ = videoPlayer_.EffectivePlaybackRate();
...
private:
    void ScheduleVideoPlaybackTimer(bool resetDeadline = false) {
        ...
        QueryPerformanceCounter(&now);
        QueryPerformanceFrequency(&frequency);
        const double rate = std::clamp(std::abs(videoPlayer_.EffectivePlaybackRate()), 0.25, 2.0);
...
private:
    ...
    void DrawZoomHud(const ZoomHudLayout& hud, float physicalScale, float opacity, bool includeAdjustmentButton, bool adjustmentsHighlighted) {
        ...
        if (FAILED(renderTarget_->CreateSolidColorBrush(D2D1::ColorF(0.0f, 0.0f, 0.0f, 0.50f * opacity), &backing)) ||
            FAILED(renderTarget_->CreateSolidColorBrush(D2D1::ColorF(1.0f, 1.0f, 1.0f, 0.62f * opacity), &text)) ||
        ...
        if (hud.hasZoom && EnsureZoomHudFormat()) {
            wchar_t label[16]{};
            const float percent = physicalScale * 100.0f;
    ...
    void DrawModelViewBar() {
        ...
        if(buildPlateSizePopupOpen_&&BuildPlateVisible()){const RECT popup=GetBuildPlateSizePopupBounds(),linkBounds=GetBuildPlateSizeLinkBounds();const int row=MulDiv(30,GetDpiForWindow(window_),96);const D2D1_RECT_F r=D2D1::RectF((float)popup.left,(float)popup.top,(float)popup.right,(float)popup.bottom);renderTarget_->FillRoundedRectangle(D2D1::RoundedRect(r,6*dpi,6*dpi),surface.Get());renderTarget_->DrawRoundedRectangle(D2D1::RoundedRect(r,6*dpi,6*dpi),border.Get(),1);DrawOverlayText(L"Build plate size",r.left+12*dpi,r.top,r.right-r.left-24*dpi,(float)row,14,DWRITE_FONT_WEIGHT_SEMI_BOLD,text.Get(),true);const auto sizeRow=[&](int index,ButtonKind button,const wchar_t* name,float value){const D2D1_RECT_F item=D2D1::RectF(r.left,(float)(popup.top+row*index),r.right,(float)(popup.top+row*(index+1)));const RECT field=GetBuildPlateSizeFieldBounds(index);const D2D1_RECT_F valueBox=D2D1::RectF((float)field.left,(float)field.top,(float)field.right,(float)field.bottom);wchar_t display[32]{};if(buildPlateSizeEdit_==index)swprintf_s(display,L"%s",buildPlateSizeText_.c_str());else swprintf_s(display,L"%.0f",value);DrawOverlayText(name,item.left+12*dpi,item.top,(float)(field.left-popup.left)-20*dpi,item.bottom-item.top,14,DWRITE_FONT_WEIGHT_NORMAL,text.Get(),true);if(buildPlateSizeEdit_==index&&buildPlateSizeReplace_)renderTarget_->FillRoundedRectangle(D2D1::RoundedRect(valueBox,3*dpi,3*dpi),selected.Get());else renderTarget_->FillRoundedRectangle(D2D1::RoundedRect(valueBox,3*dpi,3*dpi),(hoveredButton_==button||pressedButton_==button)?hover.Get():surface.Get());renderTarget_->DrawRoundedRectangle(D2D1::RoundedRect(valueBox,3*dpi,3*dpi),buildPlateSizeEdit_==index?selected.Get():border.Get(),1);DrawOverlayText(display,valueBox.left+6*dpi,valueBox.top,valueBox.right-valueBox.left-34*dpi,valueBox.bottom-valueBox.top,14,DWRITE_FONT_WEIGHT_SEMI_BOLD,text.Get(),true);DrawOverlayText(L"mm",valueBox.right-28*dpi,valueBox.top,22*dpi,valueBox.bottom-valueBox.top,12,DWRITE_FONT_WEIGHT_NORMAL,text.Get(),true);if(buildPlateSizeEdit_==index)renderTarget_->DrawLine(D2D1::Point2F(valueBox.left+6*dpi,valueBox.top+5*dpi),D2D1::Point2F(valueBox.left+6*dpi,valueBox.bottom-5*dpi),text.Get(),1);};sizeRow(1,ButtonKind::ViewBarPlateWidth,L"Width",modelBuildPlateWidthMm_);sizeRow(2,ButtonKind::ViewBarPlateDepth,L"Depth",modelBuildPlateDepthMm_);const D2D1_RECT_F link=D2D1::RectF((float)linkBounds.left,(float)linkBounds.top,(float)linkBounds.right,(float)linkBounds.bottom);const bool linkHover=hoveredButton_==ButtonKind::ViewBarPlateLink||pressedButton_==ButtonKind::ViewBarPlateLink;if(linkHover)renderTarget_->FillRoundedRectangle(D2D1::RoundedRect(link,3*dpi,3*dpi),hover.Get());const float lockX=(link.left+link.right)*.5f,lockTop=(link.top+link.bottom)*.5f-7*dpi;const D2D1_ROUNDED_RECT body=D2D1::RoundedRect(D2D1::RectF(lockX-8*dpi,lockTop+9*dpi,lockX+8*dpi,lockTop+21*dpi),2*dpi,2*dpi);renderTarget_->FillRoundedRectangle(body,surface.Get());ComPtr<ID2D1PathGeometry> shackle;ComPtr<ID2D1GeometrySink> shackleSink;if(SUCCEEDED(d2dFactory_->CreatePathGeometry(&shackle))&&SUCCEEDED(shackle->Open(&shackleSink))){shackleSink->BeginFigure(D2D1::Point2F(lockX-5*dpi,lockTop+10*dpi),D2D1_FIGURE_BEGIN_HOLLOW);shackleSink->AddLine(D2D1::Point2F(lockX-5*dpi,lockTop+5*dpi));shackleSink->AddQuadraticBezier(D2D1::QuadraticBezierSegment(D2D1::Point2F(lockX-5*dpi,lockTop),D2D1::Point2F(lockX,lockTop)));if(buildPlateSizeLinked_){shackleSink->AddQuadraticBezier(D2D1::QuadraticBezierSegment(D2D1::Point2F(lockX+5*dpi,lockTop),D2D1::Point2F(lockX+5*dpi,lockTop+5*dpi)));shackleSink->AddLine(D2D1::Point2F(lockX+5*dpi,lockTop+10*dpi));}else{shackleSink->AddQuadraticBezier(D2D1::QuadraticBezierSegment(D2D1::Point2F(lockX+4*dpi,lockTop),D2D1::Point2F(lockX+7*dpi,lockTop+3*dpi)));shackleSink->AddLine(D2D1::Point2F(lockX+9*dpi,lockTop+1*dpi));}shackleSink->EndFigure(D2D1_FIGURE_END_OPEN);if(SUCCEEDED(shackleSink->Close()))renderTarget_->DrawGeometry(shackle.Get(),text.Get(),1.5f);}renderTarget_->DrawRoundedRectangle(body,text.Get(),1.5f);const D2D1_ELLIPSE keyhole=D2D1::Ellipse(D2D1::Point2F(lockX,lockTop+14*dpi),1.5f*dpi,1.5f*dpi);renderTarget_->FillEllipse(keyhole,text.Get());renderTarget_->DrawLine(D2D1::Point2F(lockX,lockTop+15*dpi),D2D1::Point2F(lockX,lockTop+18*dpi),text.Get(),1.5f);if(linkHover){const float tooltipWidth=132.f*dpi,tooltipHeight=26.f*dpi;const D2D1_RECT_F tooltip=D2D1::RectF(link.right+5*dpi,link.top+(link.bottom-link.top-tooltipHeight)*.5f,link.right+5*dpi+tooltipWidth,link.top+(link.bottom-link.top+tooltipHeight)*.5f);renderTarget_->FillRoundedRectangle(D2D1::RoundedRect(tooltip,4*dpi,4*dpi),surface.Get());renderTarget_->DrawRoundedRectangle(D2D1::RoundedRect(tooltip,4*dpi,4*dpi),border.Get(),1);DrawOverlayText(L"Link width and depth",tooltip.left+8*dpi,tooltip.top,tooltip.right-tooltip.left-16*dpi,tooltip.bottom-tooltip.top,12,DWRITE_FONT_WEIGHT_NORMAL,text.Get(),true);}const D2D1_RECT_F reset=D2D1::RectF(r.left,(float)(popup.top+row*3),r.right,r.bottom);if(hoveredButton_==ButtonKind::ViewBarPlateReset||pressedButton_==ButtonKind::ViewBarPlateReset)renderTarget_->FillRectangle(reset,hover.Get());DrawOverlayText(L"Reset to 256 \u00D7 256 mm",reset.left+12*dpi,reset.top,reset.right-reset.left-24*dpi,reset.bottom-reset.top,14,DWRITE_FONT_WEIGHT_NORMAL,text.Get(),true);}
    }
    void DrawTitleBar() {
        if (fullscreen_) return;
    ...
    int filmstripHoverVisualIndex_ = -1;
    float filmstripHoverVisualProgress_ = 0.0f;
    float filmstripHoverVisualStartProgress_ = 0.0f;
    float filmstripHoverVisualTargetProgress_ = 0.0f;
    LONGLONG filmstripHoverVisualStartedQpc_ = 0;
    LONGLONG filmstripHoverVisualQpcFrequency_ = 0;
    bool filmstripHoverVisualAnimating_ = false;
...
LRESULT CALLBACK WindowProc(HWND window, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_LBUTTONDOWN: {
        ...
        if (viewer->BuildPlateSizePopupOpen()) {
            if (!viewer->BuildPlateSizePopupContains(point)) { viewer->CommitActiveBuildPlateSize(); viewer->DismissBuildPlateSizePopup(); return 0; }
            const ButtonKind button = viewer->ButtonAt(point);
            if (button != ButtonKind::None) { viewer->SetButtonPressed(button); SetCapture(window); }
            else viewer->CommitActiveBuildPlateSize();
            return 0;
        }
    ...
    case WM_TIMER:
        ...
        if (wParam == kFilmstripWrapFadeTimer) { viewer->UpdateFilmstripWrapFade(); return 0; }
