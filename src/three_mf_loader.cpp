...
struct SourceTriangle { uint32_t first = 0, second = 0, third = 0, slicerExtruder = 0; PropertyReference property, corners[3]; Float3 colors[3]{}; bool hasColors[3]{}; };
struct SourceComponent { uint32_t objectId = 0; Matrix4 transform = Matrix4::Identity(); std::wstring partPath, name; };
struct SourceObject { uint32_t id = 0; std::wstring name; PropertyReference property; std::vector<Float3> vertices; std::vector<SourceTriangle> triangles; std::vector<SourceComponent> components; };
struct BuildItem { uint32_t objectId = 0; Matrix4 transform = Matrix4::Identity(); std::wstring name; };
...
uint32_t DecodeSimpleSlicerExtruder(const std::wstring& value) { uint32_t state=0; return ParseUnsigned(value,state) && state && state%4==0 ? state/4 : 0; }
...
bool ParseModelXml(IStream* stream, ParsedModel& model, bool requireBuild, std::wstring& error) {
    ...
    while (reader->Read(&type) == S_OK) {
        if (type == XmlNodeType_Element) {
            ...
            else if (wcscmp(local,L"triangle") == 0 && currentObject) { auto object=model.objects.find(currentObject); SourceTriangle triangle{}; if (object==model.objects.end() || !ParseUnsigned(Attribute(reader.Get(),L"v1"),triangle.first) || !ParseUnsigned(Attribute(reader.Get(),L"v2"),triangle.second) || !ParseUnsigned(Attribute(reader.Get(),L"v3"),triangle.third) || object->second.triangles.size()>=kMaxTriangles) { error=L"The 3MF contains an invalid triangle."; return false; } triangle.property=Property(reader.Get()); triangle.corners[0]=Property(reader.Get(),L"p1"); triangle.corners[1]=Property(reader.Get(),L"p2"); triangle.corners[2]=Property(reader.Get(),L"p3"); triangle.slicerExtruder=DecodeSimpleSlicerExtruder(Attribute(reader.Get(),L"paint_color")); if(!triangle.slicerExtruder) triangle.slicerExtruder=DecodeSimpleSlicerExtruder(Attribute(reader.Get(),L"mmu_segmentation")); object->second.triangles.push_back(triangle); }
            else if (wcscmp(local,L"component") == 0 && currentObject) { auto object=model.objects.find(currentObject); SourceComponent component{}; component.partPath=Attribute(reader.Get(),L"path"); component.name=Attribute(reader.Get(),L"name"); if (object==model.objects.end()) { error=L"The 3MF contains an invalid component."; return false; } if (!ParseUnsigned(Attribute(reader.Get(),L"objectid"),component.objectId) || component.objectId==0 || !ParseTransform(Attribute(reader.Get(),L"transform"),component.transform)) continue; if(!component.partPath.empty()) model.referencedPartPaths.insert(component.partPath); object->second.components.push_back(std::move(component)); }
            else if (wcscmp(local,L"item") == 0) { BuildItem item{}; item.name=Attribute(reader.Get(),L"name"); if (!ParseUnsigned(Attribute(reader.Get(),L"objectid"),item.objectId) || item.objectId==0 || !ParseTransform(Attribute(reader.Get(),L"transform"),item.transform)) { error=L"The 3MF contains an invalid build item transform."; return false; } model.buildItems.push_back(item); }
...
std::vector<Float3> ParseFilamentPalette(const std::vector<unsigned char>& bytes) { std::string text(bytes.begin(),bytes.end()); const size_t key=text.find("\"filament_colour\""), open=key==std::string::npos?key:text.find('[',key), close=open==std::string::npos?open:text.find(']',open); std::vector<Float3> palette; if(close==std::string::npos)return palette; for(size_t cursor=open;cursor<close;){const size_t quote=text.find('"',cursor);if(quote==std::string::npos||quote>=close)break;const size_t end=text.find('"',quote+1);if(end==std::string::npos||end>close)break;std::wstring color(text.begin()+quote+1,text.begin()+end);Float3 value{};if(ParseColor(color,value))palette.push_back(value);else palette.push_back({});cursor=end+1;}return palette; }
std::unordered_map<uint32_t,uint32_t> ParseObjectExtruders(const std::vector<unsigned char>& bytes) { std::unordered_map<uint32_t,uint32_t> values; ComPtr<IStream> stream=SHCreateMemStream(bytes.data(),static_cast<UINT>(bytes.size()));ComPtr<IXmlReader> reader;if(!stream||FAILED(CreateXmlReader(IID_PPV_ARGS(&reader),nullptr))||FAILED(reader->SetInput(stream.Get())))return values;uint32_t object=0;XmlNodeType type{};while(reader->Read(&type)==S_OK){const wchar_t* local=nullptr;if(type==XmlNodeType_Element&&SUCCEEDED(reader->GetLocalName(&local,nullptr))&&local){if(wcscmp(local,L"object")==0)ParseUnsigned(Attribute(reader.Get(),L"id"),object);else if(object&&wcscmp(local,L"metadata")==0&&Attribute(reader.Get(),L"key")==L"extruder"){uint32_t extruder=0;if(ParseUnsigned(Attribute(reader.Get(),L"value"),extruder))values[object]=extruder;}}else if(type==XmlNodeType_EndElement&&SUCCEEDED(reader->GetLocalName(&local,nullptr))&&local&&wcscmp(local,L"object")==0)object=0;}return values; }

bool FlattenObject(const ParsedModel& source, uint32_t objectId, const Matrix4& transform, uint32_t buildItemIndex, uint32_t rootObjectId, uint32_t rootExtruder, const std::vector<Float3>& palette, uint32_t depth, std::unordered_set<uint32_t>& stack, MeshGeometry& mesh, ModelBounds& bounds, bool& hasBounds, std::vector<ModelInstanceRange>& ranges, std::vector<ModelComponentNode>& tree, uint32_t parentId, const std::wstring& occurrenceName, std::wstring& error) {
    ...
    stack.insert(objectId); const SourceObject& object=found->second; const uint32_t nodeId=uint32_t(tree.size()); const std::wstring& name=occurrenceName.empty()?object.name:occurrenceName; tree.push_back({nodeId,parentId,{},name.empty()?L"Object "+std::to_wstring(objectId):name,-1,objectId}); if(parentId!=UINT32_MAX)tree[parentId].children.push_back(nodeId); const uint32_t firstVertex=uint32_t(mesh.positions.size()),firstTriangle=uint32_t(mesh.indices.size()/3);
    for(const SourceTriangle& triangle:object.triangles){const auto point=[&](uint32_t index){Float3 p=TransformPoint(object.vertices[index],transform);return Float3{float(p.x*source.scaleMillimeters),float(p.y*source.scaleMillimeters),float(p.z*source.scaleMillimeters)};};const Float3 defaultColor{.72f,.75f,.80f};const uint32_t slot=triangle.slicerExtruder?triangle.slicerExtruder:rootExtruder;const Float3 slicerColor=slot&&slot<=palette.size()?palette[slot-1]:defaultColor;if(!AppendTriangle(mesh,bounds,hasBounds,point(triangle.first),point(triangle.second),point(triangle.third),triangle.hasColors[0]?triangle.colors[0]:slicerColor,triangle.hasColors[1]?triangle.colors[1]:slicerColor,triangle.hasColors[2]?triangle.colors[2]:slicerColor)){error=L"The 3MF geometry is invalid or exceeds the model limit.";stack.erase(objectId);return false;}}
...
ThreeMfLoadResult LoadThreeMfDocument(const std::wstring& path) {
    ...
    std::vector<unsigned char> projectSettings,modelSettings; std::wstring metadataError; ReadThreeMfPackagePart(path,L"Metadata/project_settings.config",projectSettings,metadataError); ReadThreeMfPackagePart(path,L"Metadata/model_settings.config",modelSettings,metadataError); const std::vector<Float3> palette=ParseFilamentPalette(projectSettings); const auto objectExtruders=ParseObjectExtruders(modelSettings);
    MeshGeometry mesh; ModelBounds bounds{}; bool hasBounds=false; std::vector<ModelInstanceRange> ranges; std::vector<ModelComponentNode> tree; for(uint32_t index=0;index<source.buildItems.size();++index){std::unordered_set<uint32_t> stack;const uint32_t root=source.buildItems[index].objectId;const auto extruder=objectExtruders.find(root);const uint32_t rootExtruder=extruder==objectExtruders.end()?0:extruder->second;if(!FlattenObject(source,root,source.buildItems[index].transform,index,root,rootExtruder,palette,0,stack,mesh,bounds,hasBounds,ranges,tree,UINT32_MAX,source.buildItems[index].name,error))return {nullptr,std::move(error)};}
    if(mesh.indices.empty()||!hasBounds) return {nullptr,L"The 3MF build contains no usable triangles."}; auto document=std::make_shared<ModelDocument>(); document->bounds=bounds; document->geometries.push_back(std::move(mesh)); document->instances.push_back({0,Matrix4::Identity()}); document->sourceFormat=ModelSourceFormat::ThreeMf; document->sourceUnit=source.unit; document->unitScaleMillimeters=source.scaleMillimeters; document->metersPerUnit=.001; document->instanceRanges=std::move(ranges); document->componentTree=std::move(tree); BuildSnapPlanes(*document);
#if defined(_DEBUG)
