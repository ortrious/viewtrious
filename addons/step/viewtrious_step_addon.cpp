...
bool LabelColor(const TDF_Label& label, ViewtriousImporterFloat4& output) {
    Quantity_ColorRGBA color; if (label.IsNull() || (!XCAFDoc_ColorTool::GetColor(label, XCAFDoc_ColorSurf, color) && !XCAFDoc_ColorTool::GetColor(label, XCAFDoc_ColorGen, color))) return false;
    const Quantity_Color& rgb = color.GetRGB(); output = {float(rgb.Red()),float(rgb.Green()),float(rgb.Blue()),color.Alpha()}; return true;
}
bool EffectiveFaceColor(const occ::handle<XCAFDoc_ColorTool>& colorTool, const occ::handle<XCAFDoc_ShapeTool>& shapeTool, const TDF_Label& occurrence, const TDF_Label& definition, const TopoDS_Shape& occurrenceShape, const TopoDS_Face& face, ViewtriousImporterFloat4& output) {
    Quantity_ColorRGBA color;
    if (colorTool->GetInstanceColor(occurrenceShape, XCAFDoc_ColorSurf, color) || colorTool->GetInstanceColor(occurrenceShape, XCAFDoc_ColorGen, color)) { const Quantity_Color& rgb=color.GetRGB(); output={float(rgb.Red()),float(rgb.Green()),float(rgb.Blue()),color.Alpha()}; return true; }
    TDF_Label faceLabel; if (shapeTool->FindSubShape(definition, face, faceLabel) && LabelColor(faceLabel, output)) return true;
    return LabelColor(occurrence, output) || LabelColor(definition, output);
}
