...
void D3D11ModelViewport::Destroy() { DiscardD3D(); document_.reset(); drag_=Drag::None; homeAnimationActive_=false; width_=height_=0; }
...
bool D3D11ModelViewport::AdvanceAnimatedHome(float progress){if(!homeAnimationActive_)return false;if(progress>=1.0f){Fit();return false;}camera_.ApplyInterpolatedAnimationState(homeAnimationStart_,homeAnimationTarget_,progress);NotifyCameraChanged();return true;}
