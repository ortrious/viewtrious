...
public:
    ...
    void FitModel() { if (ModelActive()) { modelViewport_.ClearSelectedSnapPlane(); CancelAnimatedModelHome(); modelViewport_.Fit(); InvalidateRect(window_, nullptr, FALSE); } }
    void BeginAnimatedModelHome() { if (!ModelActive()) return; modelViewport_.ClearSelectedSnapPlane(); if (!modelViewport_.BeginAnimatedHome()) return; modelHomeAnimationStartMs_ = GetTickCount64(); SetTimer(window_, kModelHomeAnimationTimer, 16, nullptr); InvalidateRect(window_, nullptr, FALSE); }
    ...
    void BeginModelOrbit(POINT point) { if (ModelActive()) { modelViewport_.ClearSelectedSnapPlane(); CancelAnimatedModelHome(); modelViewport_.BeginOrbit(point); } }
    void BeginModelPan(POINT point) { if (ModelActive()) { modelViewport_.ClearSelectedSnapPlane(); CancelAnimatedModelHome(); modelViewport_.BeginPan(point); } }
    ...
    void DismissContextMenu() {
        ...
        contextMenuOpen_ = false; if (ModelActive()) { modelFaceSelected_ = false; modelViewport_.ClearSelectedSnapPlane(); }
    ...
    void InvokeContextAction(ContextAction action) {
        if(action==ContextAction::SnapViewToFace){const Float3 normal=selectedFaceNormal_;const OrbitCamera::State state=modelViewport_.Camera().NavLibState();const auto dot=[](Float3 a,Float3 b){return a.x*b.x+a.y*b.y+a.z*b.z;};const auto normalize=[&](Float3 v){const float length=std::sqrt(dot(v,v));return length>1e-6f?Float3{v.x/length,v.y/length,v.z/length}:Float3{0,1,0};};const float side=dot(normal,state.forward);const Float3 forward=side<0?Float3{-normal.x,-normal.y,-normal.z}:normal;Float3 up{state.up.x-forward.x*dot(state.up,forward),state.up.y-forward.y*dot(state.up,forward),state.up.z-forward.z*dot(state.up,forward)};if(dot(up,up)<1e-6f){const Float3 fallback=std::fabs(forward.y)<.95f?Float3{0,1,0}:Float3{1,0,0};up={fallback.x-forward.x*dot(fallback,forward),fallback.y-forward.y*dot(fallback,forward),fallback.z-forward.z*dot(fallback,forward)};}DismissContextMenu();BeginAnimatedModelOrientation(forward,normalize(up));return;}
...
private:
    void DrawModelAxisIndicator() {
        ...
        const auto axis=[&](Float3 world,ID2D1Brush* brush,const wchar_t* label){const float screenX=dot(world,right),screenY=-dot(world,up),depth=-dot(world,forward);const D2D1_POINT_2F end=D2D1::Point2F(origin.x+screenX*(radius-11*dpi),origin.y+screenY*(radius-11*dpi));const float marker=std::clamp(3.5f+1.8f*(depth+1.f)*.5f,3.5f,5.3f)*dpi;renderTarget_->DrawLine(origin,end,brush,2.4f*dpi);renderTarget_->FillEllipse(D2D1::Ellipse(end,marker,marker),brush);DrawOverlayText(label,end.x-10*dpi,end.y-12*dpi,20*dpi,24*dpi,15,DWRITE_FONT_WEIGHT_SEMI_BOLD,brush,true);}; axis({1,0,0},x.Get(),L"X");axis({0,1,0},y.Get(),L"Y");axis({0,0,1},z.Get(),L"Z");renderTarget_->FillEllipse(D2D1::Ellipse(origin,3.5f*dpi,3.5f*dpi),junction.Get());
    }
...
#if defined(_DEBUG)
        RECT client{}; GetClientRect(window_, &client); wchar_t message[640]{};
        swprintf_s(message, L"Viewtrious model pick: client=(%ld,%ld) dpi=%.3f clientSize=%ldx%ld viewport=(%.1f,%.1f %.1fx%.1f) local=(%.2f,%.2f) ndc=(%.4f,%.4f) rayOrigin=(%.4f,%.4f,%.4f) rayDirection=(%.4f,%.4f,%.4f) triangle=%td plane=%d\\n", point.x, point.y, GetDpiForWindow(window_) / 96.0f, client.right-client.left, client.bottom-client.top, viewport.TopLeftX, viewport.TopLeftY, viewport.Width, viewport.Height, localX, localY, ndcX, ndcY, origin.x, origin.y, origin.z, direction.x, direction.y, direction.z, triangle, plane); OutputDebugStringW(message);
#else
