...
- Current version: `0.1.2.0` (CMake is the source of truth; the executable embeds matching VERSIONINFO metadata).
- Branch: `master`.
- Purpose: tiny Windows-native image viewer optimized for rapid startup, first presentation, and shutdown.
...
- Canonical executables: `out\debug\mediaView.exe` and `out\release\mediaView.exe`. Do not change these paths between versions.
- Controls: Left/Right navigate siblings; wheel and `+`/`-` zoom; `0` fits; left drag pans a manually zoomed image; drop replaces; Esc exits.
- Placement: normal rectangle and maximized state persist under `HKCU\Software\mediaView`; invalid off-screen placements are moved onto the nearest monitor.
