...
bool FlattenObject(const ParsedModel& source, uint32_t objectId, const Matrix4& transform, uint32_t buildItemIndex, uint32_t rootObjectId, uint32_t rootExtruder, const std::vector<Float3>& palette, uint32_t depth, std::unordered_set<uint32_t>& stack, MeshGeometry& mesh, ModelBounds& bounds, bool& hasBounds, std::vector<ModelInstanceRange>& ranges, std::wstring& error) {
    ...
    for(const SourceComponent& component:object.components)if(!FlattenObject(source,component.objectId,Multiply(component.transform,transform),buildItemIndex,rootObjectId,rootExtruder,palette,depth+1,stack,mesh,bounds,hasBounds,ranges,error)){stack.erase(objectId);return false;}
...
ThreeMfLoadResult LoadThreeMfDocument(const std::wstring& path) {
    ...
    MeshGeometry mesh; ModelBounds bounds{}; bool hasBounds=false; std::vector<ModelInstanceRange> ranges; for(uint32_t index=0;index<source.buildItems.size();++index){std::unordered_set<uint32_t> stack;const uint32_t root=source.buildItems[index].objectId;const auto extruder=objectExtruders.find(root);const uint32_t rootExtruder=extruder==objectExtruders.end()?0:extruder->second;if(!FlattenObject(source,root,source.buildItems[index].transform,index,root,rootExtruder,palette,0,stack,mesh,bounds,hasBounds,ranges,error))return {nullptr,std::move(error)};}
