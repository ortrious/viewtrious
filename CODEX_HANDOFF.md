...
- Current version: `0.2.2.0` (CMake is the source of truth; the executable embeds matching VERSIONINFO metadata).
- Branch: `master`.
...
- Top bar: a 40-DIP custom title bar contains a full-height hamburger, decoded resolution, cheap `GetFileAttributesExW` file-size metadata, and an ellipsized filename, followed by custom caption controls. Resolution and file size use stable centered slots; the filename takes remaining width and is trimmed first. The hamburger opens the native `Keyboard Shortcuts`, separator, `About`, `Close` popup.
- Overlays: Keyboard Shortcuts and About are compact, centered, theme-aware Direct2D panels. Esc closes an open overlay before fullscreen exit or app close, and overlay input does not reach viewer pan, zoom, navigation, or fullscreen actions. The About logo is decoded only when that overlay opens.
