...
bool D3D11ModelViewport::Create(HWND parent, const RECT& bounds, std::shared_ptr<ModelDocument> document, std::wstring& error) {
    ...
    WNDCLASSW wc{}; wc.lpfnWndProc=WindowProc; wc.hInstance=GetModuleHandleW(nullptr); wc.lpszClassName=kViewportClass; wc.hCursor=LoadCursorW(nullptr,IDC_ARROW); static const ATOM atom=RegisterClassW(&wc); (void)atom;
    ...
    if(!window_){error=L"The 3D viewport could not be created."; Destroy(); return false;} if(!InitializeD3D(error) || !UploadModel(error)){Destroy();return false;} Fit(); Render(); return true;
}
...
bool D3D11ModelViewport::InitializeD3D(std::wstring& error) {
    resources_=std::make_unique<Resources>(); UINT flags=D3D11_CREATE_DEVICE_BGRA_SUPPORT; D3D_FEATURE_LEVEL level{}; const D3D_FEATURE_LEVEL levels[]={D3D_FEATURE_LEVEL_11_1,D3D_FEATURE_LEVEL_11_0}; HRESULT hr=D3D11CreateDevice(nullptr,D3D_DRIVER_TYPE_HARDWARE,nullptr,flags,levels,2,D3D11_SDK_VERSION,&resources_->device,&level,&resources_->context); if(FAILED(hr)) hr=D3D11CreateDevice(nullptr,D3D_DRIVER_TYPE_WARP,nullptr,flags,levels,2,D3D11_SDK_VERSION,&resources_->device,&level,&resources_->context); if(FAILED(hr)){error=L"Direct3D 11 could not initialize for this model.";return false;}
    DXGI_SWAP_CHAIN_DESC desc{}; desc.BufferCount=2; desc.BufferDesc.Format=DXGI_FORMAT_B8G8R8A8_UNORM; desc.BufferUsage=DXGI_USAGE_RENDER_TARGET_OUTPUT; desc.OutputWindow=window_; desc.SampleDesc.Count=1; desc.Windowed=TRUE; desc.SwapEffect=DXGI_SWAP_EFFECT_DISCARD; ComPtr<IDXGIDevice> dxgiDevice; resources_->device.As(&dxgiDevice); ComPtr<IDXGIAdapter> adapter; dxgiDevice->GetAdapter(&adapter); ComPtr<IDXGIFactory> factory; adapter->GetParent(IID_PPV_ARGS(&factory)); if(FAILED(factory->CreateSwapChain(resources_->device.Get(),&desc,&resources_->swapChain))){error=L"The model display surface could not initialize.";return false;}
...
LRESULT CALLBACK D3D11ModelViewport::WindowProc(HWND window,UINT message,WPARAM wParam,LPARAM lParam){auto*self=reinterpret_cast<D3D11ModelViewport*>(GetWindowLongPtrW(window,GWLP_USERDATA));if(message==WM_NCCREATE){self=reinterpret_cast<D3D11ModelViewport*>(reinterpret_cast<CREATESTRUCTW*>(lParam)->lpCreateParams);SetWindowLongPtrW(window,GWLP_USERDATA,reinterpret_cast<LONG_PTR>(self));}return self?self->HandleMessage(message,wParam,lParam):DefWindowProcW(window,message,wParam,lParam);}
